...
function AccountModule({ closePopUp }) {
    // استفاده از useState برای داده‌های کاربر
    const [userData] = useState({
        name: 'Amir Ali Mohammadi',
        phone: '989123456789',
        username: '@AA_Mohammadi',
        birthDate: '', // می‌توانید تاریخ تولد را به صورت اختیاری قرار دهید
        isOnline: true
    });

    // چک کردن وجود تابع closePopUp برای جلوگیری از خطا
    ...
    // ایجاد JSX برای نمایش اطلاعات کاربر
    const userInfo = useMemo(() => {
        return (
            <>
                <div className="container-account-fullname text-capitalize">{userData.name}</div>
        ...
        <div className="container-account d-flex flex-column m-3">
            ...
            <div className="container-account-info">
                <div className="container-account-fullname d-flex flex-row justify-content-between align-items-center cursor-pointer">
                    ...
                    <div className="container-account-fullname-right text-info">{userData.name}</div>
                </div>
